...
void FirePlayerMissle(float x, float y, float z, float angy, int owner, int shoot, D3DVECTOR velocity, float lookangy, FLOAT fTimeKeysave)
{
	...
	for (misslecount = 0; misslecount < MAX_MISSLE; misslecount++)
	{
		...
		if (your_missle[misslecount].active == 1)
		{
			...
			//TODO: fix radius
			eRadius = D3DXVECTOR3(10.0f, 10.0f, 10.0f);
			...
			savevelocity.x = (savevelocity).x * 600.0f * fTimeKeysave;
			savevelocity.y = (savevelocity).y * 600.0f * fTimeKeysave;
			savevelocity.z = (savevelocity).z * 600.0f * fTimeKeysave;
			...
			eRadius = D3DXVECTOR3(25.0f, 50.0f, 25.0f);
			...
			if (foundcollisiontrue == 1 || qdist > 5000.0f)
			{
				...
				D3DXVECTOR3 a = your_missle[misslecount].velocity;

				//Vnew = b * (-2 * (V dot N) * N + V)

				D3DXVECTOR3 m1, m2, m3;
				D3DXVec3Normalize(&m1, &eTest);//surface normal
				D3DXVec3Normalize(&m2, &a);   //miss vel norm

				float fDot = D3DXVec3Dot(&m2 , &m1);
				D3DXVECTOR3 v = 1.0f * (-2.0f * (fDot) * m1 + your_missle[misslecount].velocity);

				your_missle[misslecount].velocity =   v;
				*/
...
void FireMonsterMissle(int monsterid, int type)
{
	...
	D3DXVECTOR3 MissleMove;
	D3DXVECTOR3 MissleVelocity;
	int misslecount = 0;
	int misslespot = 0;
	float gun_angle;
	float angy;
	...
	D3DXVECTOR3 scalc = slope1 - slope;
