...
void ObjectToD3DVertList(int ob_type, int angle, int oblist_index)
{
	...
	D3DXVECTOR3 v1, v2, vw1, vw2, vw3, vw4, vDiff2, vDiff3, vw5;
		...
		for (j = 0; j < num_verts_per_poly; j++)
		{
			...
			if (counttri == 2 && firstcount ==0 || firstcount ==1 )
			{
				...
				if (firstcount == 1)
				{
					...
					src_v[cnt].nx = average.x;
					src_v[cnt].ny = average.y;
					src_v[cnt].nz = average.z;

					src_v[cnt-1].nx = average.x;
					src_v[cnt-1].ny = average.y;
					src_v[cnt-1].nz = average.z;
	...
	for (int i = start_cnt;i < cnt;i++) {
		...
		if (track[i - start_cnt] == 0) {
			...
			if (scount > 0) {
				...
				for (int k = 0;k < scount;k++) {
					src_v[sharedv[k]].nx = average.x;
					src_v[sharedv[k]].ny = average.y;
					src_v[sharedv[k]].nz = average.z;
				}
			}
		}
	}
