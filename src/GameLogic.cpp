...
void MoveMonsters(float fElapsedTime)
{
	...
	D3DXVECTOR3 work1, work2, work3, vDiff, vw1, vw2;
	int i;

	D3DXVECTOR3 saveeyept, inarea, inarea2, pIPoint, inarea3;
	int cullloop2 = 0;
	int raction = 0;
	int skipmonster = 0;
	...
	for (i = 0; i < num_monsters; i++)
	{
		...
		if (cullflag)
		{
			...
			float length = D3DXVec3Length(&vDiff2);
			...
			D3DXVec3Normalize(&final, &vDiff2);
			...
			if (skipmonster == 0)
			{
				...
				*/
				...
				result = collideWithWorld(RadiusDivide(collidenow, eRadius), RadiusDivide(result, eRadius));
			...
			if (monster_list[i].current_sequence == 1)
			{
				final.x = 0.0f;
				//fixed april 2005
				final.y = (float)(-300.0f) * fElapsedTime;
				final.z = 0.0f;
				...
				result = collideWithWorld(RadiusDivide(collidenow, eRadius), RadiusDivide(final, eRadius));
				result = RadiusMultiply(result, eRadius);
...
D3DXVECTOR3 RadiusMultiply(D3DXVECTOR3  eRadius, D3DXVECTOR3 vector) {
...
int CalculateView(D3DVECTOR EyeBall, D3DVECTOR LookPoint, float angle, bool distancecheck)
{
	...
	D3DXVECTOR3 final, final2;
	D3DXVECTOR3 m, n, vw1, vw2, work2, work1, vDiff, normroadold;
	float fDot;
	float convangle;
	float anglework = 0;
	int monsteron = 0;
